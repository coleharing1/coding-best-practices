...
```text
First, read `README.md`, `WORKLOG.md`, `Coles-saved-prompts.md`, `Coles-saved-prompts-ai-optimized.md`, `research/cursor-rules-vs-claude-md-vs-agents-md.md`, and the existing templates under `templates/AGENTS.template.md`, `templates/CLAUDE.template.md`, `templates/CLAUDE.local.template.md`, `templates/GEMINI.template.md`, `templates/.cursor/rules/`, and `templates/.claude/rules/` so you understand what this repo already prescribes. Then do a focused web research pass anchored to May 2026 on current conventions for AGENTS.md, CLAUDE.md, `.cursor/rules/*.mdc`, `.claude/rules/`, Codex `AGENTS.md` precedence and `AGENTS.override.md`, Gemini CLI context files, and the AGENTS.md ↔ CLAUDE.md symlink convergence pattern (Linux Foundation Agentic AI Foundation spec, Anthropic's hierarchical memory model, Cursor's `description` / `globs` / `alwaysApply` frontmatter, and the documented "directive drift" problem where agents stop honoring persistent rules after long tool chains). Before generating any files, briefly summarize the deltas between current 2026 best practice and what this repo's templates currently teach, and flag any template updates that should follow.

Then create `AGENTS.md` and `CLAUDE.md` at the repo root for this project itself (the coding-best-practices reference repo, which currently has no root context files and must dogfood its own system). Decide explicitly whether to use a single canonical file with the other as a symlink, two distinct files, or one file plus a thin pointer — and justify the choice based on what each tool actually reads in May 2026, where Codex-style builder guidance genuinely differs from Claude-style planner/reviewer guidance, and what this repo already prescribes. Keep CLAUDE.md tight and snapshot-style (target under ~150-200 lines) and push deeper, path-scoped, or conditional guidance into `.cursor/rules/*.mdc` and `.claude/rules/` files using the conventions already established in `templates/`. Both root files must cover: the multi-model workflow (Claude plan → Codex plan → synthesized Final → Codex implement → Gemini/Claude review → quality gate → push), the macro-roadmap plus numbered-plans pattern, the repeatable-actions stack (prompts → commands → skills → rules → hooks → automations), browser QA and login-handoff expectations, the dogfooding rule that this repo follows its own system, and explicit cross-references to `README.md`, `WORKLOG.md`, `plans/MACRO-ROADMAP.md` (once present), `templates/`, `workflow/`, and `prompts/`. Avoid duplicating large blocks across files — prefer cross-references and clearly state which file owns what.

Critically, bake in self-maintenance rules in both files so the project memory stays accurate without me having to manually police it. Define explicit "evolution rules" the AI must execute at the end of any meaningful unit of work: append a new numbered entry to `WORKLOG.md` using the existing format, update `README.md` when structure or guidance changes, create or update the relevant `plans/active/Plan-XXX-*.md` and `plans/MACRO-ROADMAP.md` when scope or sequencing shifts, sync the `templates/*` source-of-truth files whenever a new convention is introduced at the root so future projects inherit it, retire stale references, and never let prompts, templates, workflow guides, and root context files drift from current reality. Mitigate the documented directive-drift problem by requiring the agent to re-read the root context files and the active plan at the start of each new phase and after long tool chains. Specify exactly when content belongs in AGENTS.md vs CLAUDE.md vs `.cursor/rules/` vs `.claude/rules/` vs `prompts/` vs `workflow/` so future contributions land in the right layer. After writing the root files, list any follow-up edits needed in `templates/` (and add a `WORKLOG.md` entry) so this repo's reusable assets stay in sync with what you just put at the root.
```
